...
const {tachanun} = require('../dist/cjs/tachanun');
const {abs2hebrew, hebrew2abs, months} = require('../dist/cjs/hdate');

test('tachanun', t => {
  const startAbs = hebrew2abs(5782, months.TISHREI, 1);
  const endAbs = hebrew2abs(5783, months.TISHREI, 1);
  const actual = [];
  for (let abs = startAbs; abs < endAbs; abs++) {
    const hd = abs2hebrew(abs);
    const tach = tachanun(hd, false);
    actual.push([abs, tach]);
  }
  const expected = [
